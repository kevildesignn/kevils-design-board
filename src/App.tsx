    ...
    const fetchGitHubImages = async () => {
      try {
        ...
        if (remoteImages.length > 0) {
          setImages((prev) => {
            // Keep inspiration cards that are not in Visual-design
            const inspirationImages = prev.filter((img) => !img.url.includes('Visual-design/'));
            return [...remoteImages, ...inspirationImages];
          });
        }
      } catch (err) {
        // Fallback silently
